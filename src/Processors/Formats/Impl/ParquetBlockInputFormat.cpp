...
static std::optional<Field> decodePlainParquetValueSlow(const std::string & data, parquet::Type::type physical_type, const parquet::ColumnDescriptor & descr)
{
    ...
    do // while (false)
    {
        ...
        if (data.size() != size || (size & (size - 1)) != 0 || size < 1 || size > 32)
            throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpected decimal size.");
...
static std::vector<Range> getHyperrectangleForRowGroup(const parquet::FileMetaData & file, size_t row_group_idx_in_file, const Block & header, const FormatSettings & format_settings)
{
    ...
    std::unique_ptr<parquet::RowGroupMetaData> row_group = file.RowGroup(static_cast<int>(row_group_idx_in_file));
    ...
    {
        auto it = name_to_statistics.find(column_name_for_lookup(header.getByPosition(idx).name));
        ...
        if (stats->HasMinMax())
        {
            ...
            {
                e.addMessage(" (When parsing Parquet statistics for column {}, physical type {}, data type {}. Please report an issue and use input_format_parquet_filter_push_down = false to work around.)");
                throw;
            }
        }
...
void ParquetBlockInputFormat::initializeIfNeeded()
{
    ...
    for (size_t idx = 0; idx < static_cast<size_t>(metadata->num_row_groups()); ++idx)
    {
        if (skip_row_groups.contains(static_cast<int>(idx)))
            continue;
...
void ParquetBlockInputFormat::initializeRowGroupReader(size_t row_group_idx)
{
    ...
    THROW_ARROW_NOT_OK(
        row_group.file_reader->GetRecordBatchReader({static_cast<int>(row_group.row_group_idx_in_file)}, column_indices, &row_group.record_batch_reader));
    ...
    auto row_group_metadata = metadata->RowGroup(static_cast<int>(row_group.row_group_idx_in_file));
    row_group.row_group_bytes_uncompressed = row_group_metadata->total_compressed_size();
    row_group.row_group_rows = row_group_metadata->num_rows();
}
